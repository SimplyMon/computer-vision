...
if id in id_names["id"].values:
    ...
    id_names = pd.concat(
        [id_names, pd.DataFrame([{"id": id, "name": name}])], ignore_index=True
    )
    id_names.to_csv("id-names.csv")
...
print(
    "Now this is where you begin taking photos. Once you see a rectangle around your face, press the 's' key to capture a picture.",
    end=" ",
)
print(
    "It is recommended to take atleast 20-25 pictures, from different angles, in different poses, with and without specs, you get the gist."
)
input(
...
while cv.waitKey(1) & 0xFF != ord("q"):
    ...
    faces = face_classifier.detectMultiScale(
        grey, scaleFactor=1.1, minNeighbors=5, minSize=(50, 50)
    )
